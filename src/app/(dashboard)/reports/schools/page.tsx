...
export default async function SchoolReportPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      <ReportHeader
        title="School Report"
        description="School-wise sales, collections, pending dues, items sold and average invoice value."
        exportHref={buildCsvExportHref("schools", filters)}
      />

      <FiltersCard>
        <form action="/reports/schools">
          <div className="grid gap-4 lg:grid-cols-[1fr_220px_180px_160px_160px_auto]">
            <input
              name="q"
              defaultValue={filters.q}
              placeholder="Search school, invoice, customer..."
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          label="Schools"
          value={String(report.summary.schoolCount)}
          hint="Schools with sales"
        />
        <StatCard
          label="Revenue"
          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/80 text-xs uppercase tracking-wide text-slate-500">
                ...
                <th className="px-6 py-4 text-right font-black">Items</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {report.rows.map((row) => (
                <tr key={row.schoolId} className="hover:bg-slate-50/80">
                  ...
                  <td className="px-6 py-5 text-right">{row.itemsSold}</td>
              ...
              {report.rows.length === 0 ? (
                <EmptyTableRow colSpan={7} label="No school records found." />
              ) : null}
            </tbody>
          </table>
        </div>
