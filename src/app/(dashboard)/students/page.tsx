import { createStudentAction, toggleStudentAction, updateStudentAction } from "@/features/students/actions";
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm">
        ...
        <tbody className="divide-y divide-slate-100">{students.map(student => <tr key={student.id}>
          <td className="px-4 py-3 font-medium text-slate-900">{student.name}</td><td className="px-4 py-3">{student.school.name}</td><td className="px-4 py-3">{student.className}{student.sectionName ? ` / ${student.sectionName}` : ""}</td><td className="px-4 py-3">{student.parentPhone || "-"}</td><td className="px-4 py-3">{student.admissionNo || "-"} / {student.rollNumber || "-"}</td><td className="px-4 py-3">{student.isActive ? "Active" : "Inactive"}</td><td className="px-4 py-3">{hasPermission(access, Permission.MANAGE_STUDENTS, student.schoolId) ? <div className="flex items-start gap-3"><details><summary className="cursor-pointer text-sm font-semibold text-blue-700 underline">Edit</summary><form action={updateStudentAction} className="mt-2 grid min-w-[560px] gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 md:grid-cols-2"><input type="hidden" name="studentId" value={student.id}/><input name="name" defaultValue={student.name} required placeholder="Student name *" className="rounded-lg border border-slate-300 px-3 py-2 text-sm"/><input name="className" defaultValue={student.className} required placeholder="Class *" className="rounded-lg border border-slate-300 px-3 py-2 text-sm"/><input name="parentPhone" defaultValue={student.parentPhone || ""} required placeholder="Contact number *" className="rounded-lg border border-slate-300 px-3 py-2 text-sm"/><input name="sectionName" defaultValue={student.sectionName || ""} placeholder="Section" className="rounded-lg border border-slate-300 px-3 py-2 text-sm"/><input name="admissionNo" defaultValue={student.admissionNo || ""} placeholder="Admission no." className="rounded-lg border border-slate-300 px-3 py-2 text-sm"/><input name="rollNumber" defaultValue={student.rollNumber || ""} placeholder="Roll no." className="rounded-lg border border-slate-300 px-3 py-2 text-sm"/><input name="parentName" defaultValue={student.parentName || ""} placeholder="Parent / guardian" className="rounded-lg border border-slate-300 px-3 py-2 text-sm"/><input name="address" defaultValue={student.address || ""} placeholder="Address" className="rounded-lg border border-slate-300 px-3 py-2 text-sm"/><button className="rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white md:col-span-2">Save Changes</button></form></details><form action={toggleStudentAction}><input type="hidden" name="studentId" value={student.id}/><button className="text-sm font-semibold text-slate-700 underline">{student.isActive ? "Deactivate" : "Restore"}</button></form></div> : "-"}</td>
        </tr>)}</tbody>
      </table></div>
